...
export default function DriverDashboard() {
  ...
  const logout = async () => { await supabase.auth.signOut(); router.replace('/driver/login'); };
  ...
  return <main className="bg-mist"><div className="container py-12">
    ...
    <section className="mt-10 rounded-2xl bg-white p-6 shadow-sm"><h2 className="text-xl font-black">My bookings</h2>{bookings.length === 0 ? <p className="mt-5 rounded-xl border border-dashed border-slate-200 px-5 py-10 text-center text-sm text-slate-500">No bookings assigned to you yet.</p> : <div className="mt-5 divide-y divide-slate-100">{bookings.map(booking => <div key={booking.id} className="flex flex-col justify-between gap-2 py-5 sm:flex-row"><div><p className="font-bold">{booking.from_area}{booking.to_area ? ` to ${booking.to_area}` : ''}</p><p className="mt-1 text-sm text-slate-500">{booking.service_type} · {booking.name}</p></div><div className="text-left sm:text-right"><p className="font-black">R{Number(booking.price || 0).toLocaleString()}</p><p className="mt-1 text-xs font-bold uppercase text-slate-400">{booking.status}</p></div></div>)}</div>}</section>
  </div></main>;
}